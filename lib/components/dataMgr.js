...
pro.selectData = function (tableName, priValue, forValue, cb) {
    ...
    self.tableRedis.getRedisCache(tableName, priValue, forValue, (err, data)=>{
        ...
        // 从mysql取数据的时候,切记要根据外键值取,才能将全部数据放入redis,若直接根据主键取,那么缓存里的数据就只有一条.
        var sql = self.dbTable.getSelectSql(tableName, 0, forValue);
        if (!sql) {
        ...
        self.dbMgr.query(sql, [], function (err, dataDB) {
            if (!!err) {
                ...
                self.tableRedis.addRedisCache(tableName, dataDB, (err)=>{
                    ...
                    if (!!priValue) {
                        res = null;
                        var priKey = self.getPrimaryKey(tableName);
                        for (let c = 0; c < dataDB.length; c++) {
                            if (priValue == dataDB[c][priKey]) {
                                res = dataDB[c];
                                break;
                            }
                        }
